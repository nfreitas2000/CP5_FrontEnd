...
export default function Funcionarios(){
    ...
    return(
        <div className="w-3/4 mt-8 m-auto size-svh">
            <h1 className="text-blue-800 text-5xl text-center font-bold mb-8">Lista de Funcionários</h1>

            <Link className="p-2.5 bg-green-500 text-white font-bold rounded-md" to={'/inserir-funcionario'}>Inserir Funcionário</Link>
                ...
                <tbody>
                    {
                        funcionario.map(f=>(
                            <tr className="even:bg-gray-400 *:text-center *:p-2.5" key={f.id}>
                                <td>{f.nome}</td><td>{f.cargo}</td><td>{f.setor}</td><td>{f.turno}</td><td>R${f.salario.toFixed(2)}</td>
                                <td>
                                    <Link className="m-1 bg-blue-600 text-white px-2 
                                    pb-1 rounded-md hover:font-bold" to={`/funcionario/${f.id}`}>Editar</Link>
                                    <button onClick={() => handleDelete(f.id)} className="m-1 bg-red-600 text-white px-2 
